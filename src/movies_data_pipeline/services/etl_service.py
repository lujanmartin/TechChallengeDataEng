...
class ETLService:
    def __init__(self):
        self.bronze_file_path = (
            Path(os.getenv("BRONZE_BASE_PATH")) / "bronze_movies.parquet"
        )
    ...
    def _update_silver_with_bronze_ids(self, bronze_ids: List[int]):
        try:
            ...
            updated_bronze_df = self.transformer._process_genre_and_crew(
                updated_bronze_df
            )
            ...
            if not silver_df.empty and "bronze_id" in silver_df.columns:
                for _, updated_row in updated_bronze_df.iterrows():
                    ...
                    if mask.any():
                        ...
                        max_silver_id = (
                            silver_df["silver_id"].max()
                            if "silver_id" in silver_df.columns
                            else 0
                        )
                        updated_row_df = pd.DataFrame([updated_row])
                        updated_row_df["silver_id"] = max_silver_id + 1
                        silver_df = pd.concat(
                            [silver_df, updated_row_df], ignore_index=True
                        )
            else:
                max_silver_id = 0
                updated_bronze_df["silver_id"] = range(
                    max_silver_id + 1, max_silver_id + 1 + len(updated_bronze_df)
                )
            ...
            logger.info(
                f"Updated silver layer with {len(updated_bronze_df)} records for bronze_ids {bronze_ids}"
            )
    ...
    def _delete_from_silver(self, bronze_ids: List[int]):
        try:
            ...
            remaining_ids = (
                [bid for bid in bronze_ids if bid not in silver_df["bronze_id"].values]
                if "bronze_id" in silver_df.columns
                else bronze_ids
            )
            if remaining_ids:
                ...
                if not bronze_records.empty:
                    for _, row in bronze_records.iterrows():
                        ...
                        mask = (silver_df["name"] == name) & (
                            silver_df["orig_title"] == orig_title
                        )
                        if mask.any():
                            silver_df = silver_df[~mask]
                            deleted_count += mask.sum()
                            logger.info(
                                f"Deleted record with name '{name}' and orig_title '{orig_title}' from silver"
                            )
                        else:
                            logger.info(
                                f"No match in silver for bronze_id {row['bronze_id']}, name '{name}', orig_title '{orig_title}'"
                            )
                else:
                    logger.info(
                        f"No records found in bronze for remaining bronze_ids {remaining_ids}"
                    )
            ...
            if deleted_count > 0:
                ...
                logger.info(
                    f"No records deleted from silver for bronze_ids {bronze_ids}"
                )
    ...
    def _run_full_etl(self):
        try:
            ...
            gold_tables = self.transformer._create_gold_tables(
                silver_df, lineage_entries, str(self.bronze_file_path)
            )
