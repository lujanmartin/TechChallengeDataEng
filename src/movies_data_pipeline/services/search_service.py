...
class SearchService:
    ...
    def search_movies(
        self, query: str, limit: int = 10, offset: int = 0, genre: str = None
    ) -> List[Movie]:
        page = (offset // limit) + 1
        hits = self.vector_db.search_movies(
            query, per_page=limit, page=page, genre_filter=genre
        )
        ...
        for hit in hits:
            ...
            try:
                release_date = (
                    date.fromisoformat(release_date_str.split("T")[0])
                    if release_date_str
                    else None
                )
