...
class Transformer:
    ...
    def transform(self, new_file_path: str) -> Dict[str, pd.DataFrame]:
        ...
        gold_tables = self._create_gold_tables(new_silver_df, lineage_entries, new_file_path)
    ...
    def _create_silver_layer(self, new_df: pd.DataFrame, new_file_path: str) -> tuple[pd.DataFrame, pd.DataFrame, List[Dict[str, Any]]]:
        ...
        if self.silver_file_path.exists():
            ...
            start_id = existing_silver["silver_id"].max() + 1 if not existing_silver.empty else 0
        else:
        ...
        new_silver_df = silver_df if existing_silver.empty else silver_df[
            ~silver_df[unique_key].apply(lambda x: tuple(x), axis=1).isin(
    ...
    def _create_gold_tables(self, silver_df: pd.DataFrame, lineage_entries: List[Dict[str, Any]], new_file_path: str) -> Dict[str, pd.DataFrame]:
        ...
        dim_country = silver_df[["country"]].drop_duplicates().rename(columns={"country": "country_name"})
        ...
        dim_language = silver_df[["orig_lang"]].drop_duplicates().rename(columns={"orig_lang": "language_name"})
        ...
        dim_crew = pd.DataFrame(crew_flat["crew_pairs"].tolist()).drop_duplicates(subset=["actor_name", "character_name"])
        ...
        dim_genre = silver_df.explode("genre_list")[["genre_list"]].drop_duplicates().rename(columns={"genre_list": "genre_name"})
        ...
        bridge_movie_genre = bridge_movie_genre.merge(dim_movie[["name", "orig_title", "lineage_id"]], on=["name", "orig_title"], how="left").rename(columns={"lineage_id": "movie_lineage_id"})
        bridge_movie_genre = bridge_movie_genre.merge(dim_genre[["genre_name", "lineage_id"]], left_on="genre_list", right_on="genre_name", how="left").rename(columns={"lineage_id": "genre_lineage_id"})
        bridge_movie_genre["lineage_id"] = bridge_movie_genre["movie_lineage_id"] + "_" + bridge_movie_genre["genre_lineage_id"]
        bridge_movie_genre["created_at"] = bridge_movie_genre["updated_at"] = current_time
        bridge_movie_genre = bridge_movie_genre[["movie_lineage_id", "genre_lineage_id", "lineage_id", "created_at", "updated_at"]].drop_duplicates(subset=["movie_lineage_id", "genre_lineage_id"])
        ...
        bridge_movie_crew = silver_df.explode("crew_pairs")[["name", "orig_title", "crew_pairs"]].dropna(subset=["crew_pairs"]).reset_index(drop=True)
        ...
        bridge_movie_crew = bridge_movie_crew.merge(dim_movie[["name", "orig_title", "lineage_id"]], on=["name", "orig_title"], how="left").rename(columns={"lineage_id": "movie_lineage_id"})
        bridge_movie_crew = bridge_movie_crew.merge(dim_crew[["actor_name", "character_name", "lineage_id"]], on=["actor_name", "character_name"], how="left").rename(columns={"lineage_id": "crew_lineage_id"})
        ...
        bridge_movie_crew = bridge_movie_crew[["movie_lineage_id", "crew_lineage_id", "character_name", "lineage_id", "created_at", "updated_at"]].drop_duplicates(subset=["movie_lineage_id", "crew_lineage_id", "character_name"])
        ...
        fact_movie_metrics = silver_df[["silver_id", "name", "orig_title", "release_date", "country", "orig_lang", "budget_x", "revenue", "score"]]
        fact_movie_metrics = fact_movie_metrics.merge(dim_movie[["name", "orig_title", "lineage_id"]], on=["name", "orig_title"], how="left").rename(columns={"lineage_id": "movie_lineage_id"})
        fact_movie_metrics = fact_movie_metrics.merge(dim_date[["release_date", "lineage_id"]], on="release_date", how="left").rename(columns={"lineage_id": "date_lineage_id"})
        fact_movie_metrics = fact_movie_metrics.merge(dim_country[["country_name", "lineage_id"]], left_on="country", right_on="country_name", how="left").rename(columns={"lineage_id": "country_lineage_id"})
        fact_movie_metrics = fact_movie_metrics.merge(dim_language[["language_name", "lineage_id"]], left_on="orig_lang", right_on="language_name", how="left").rename(columns={"lineage_id": "language_lineage_id"})
        fact_movie_metrics["lineage_id"] = (
            fact_movie_metrics["silver_id"].astype(str) + "_" + fact_movie_metrics["movie_lineage_id"] + "_" + 
            fact_movie_metrics["date_lineage_id"] + "_" + fact_movie_metrics["country_lineage_id"] + "_" + 
            fact_movie_metrics["language_lineage_id"]
        )
        fact_movie_metrics["created_at"] = fact_movie_metrics["updated_at"] = current_time
        fact_movie_metrics = fact_movie_metrics.rename(columns={"budget_x": "budget"})
        fact_movie_metrics = fact_movie_metrics[["silver_id", "movie_lineage_id", "date_lineage_id", "country_lineage_id", "language_lineage_id", "budget", "revenue", "score", "lineage_id", "created_at", "updated_at"]].drop_duplicates(subset=["silver_id"])
        ...
        revenue_by_genre = temp_movie_revenue.merge(temp_bridge, on="movie_lineage_id").merge(dim_genre[["genre_name", "lineage_id"]], left_on="genre_lineage_id", right_on="lineage_id").groupby("genre_name")["revenue"].sum().reset_index().rename(columns={"revenue": "total_revenue"})
        ...
        avg_score_by_year = fact_movie_metrics.merge(dim_date[["lineage_id", "year"]], left_on="date_lineage_id", right_on="lineage_id").groupby("year")["score"].mean().reset_index().rename(columns={"score": "avg_score"})
        ...
        lineage_df = pd.DataFrame(lineage_entries).rename(columns={"record_id": "lineage_id"})
