...
class SearchServiceAdapter:
    ...
    def batch_create_documents(
        ...
        chunks = [
            processed_movies[i : i + chunk_size]
            for i in range(0, len(processed_movies), chunk_size)
        ]
        ...
        for chunk in chunks:
            thread = threading.Thread(
                target=self._index_chunk, args=(chunk, batch_size, exceptions)
            )
        ...
        if exceptions:
            raise Exception(
                "Errors in parallel indexing: " + "; ".join(str(e) for e in exceptions)
            )

        logger.info(
            f"Batch indexed {len(processed_movies)} movies with {len(threads)} threads"
        )

    def _index_chunk(
        self, chunk: List[Dict[str, Any]], batch_size: int, exceptions: List[Exception]
    ) -> None:
        try:
    ...
    def _prepare_movie(self, movie_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "id": str(
                row.get("movie_id", row["bronze_id"])
            ),  # Fallback to bronze_id if no movie_id
            "name": row["name"],
    ...
    def _prepare_movies_bulk(
        self, movie_data_list: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
