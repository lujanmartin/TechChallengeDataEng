...
class VectorDB:
    ...
    def _initialize_collection(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Deleted existing Typesense collection: {self.collection_name}"
            )
        except Exception:
        ...
        try:
            ...
            for movie in movies_query:
                genres = (
                    ...
                    .join(
                        BridgeMovieGenre, BridgeMovieGenre.genre_id == DimGenre.genre_id
                    )
                ...
                crew_list = [
                    {"actor_name": c.actor_name, "character_name": c.character_name}
                    for c in crew
                ]

                metrics = (
                    self.db_session.query(
                        DimDate.release_date, DimLanguage.language_name
                    )
                    .join(FactMovieMetrics, FactMovieMetrics.date_id == DimDate.date_id)
                    .join(
                        DimLanguage,
                        FactMovieMetrics.language_id == DimLanguage.language_id,
                    )
                    .filter(FactMovieMetrics.movie_id == movie.movie_id)
                    .first()
                )
                release_date = (
                    metrics.release_date if metrics and metrics.release_date else None
                )
                if release_date and " " in release_date:
                    release_date = release_date.split(" ")[0]  # e.g., "2023-03-02"

                language = (
                    metrics.language_name if metrics and metrics.language_name else None
                )
                ...
                if len(documents) >= batch_size:
                    ...
                    logger.debug(
                        f"Synced batch of {len(documents)} movies; total synced: {total_synced}"
                    )
                    documents = []
            ...
            if documents:
                ...
                total_synced += len(documents)
                logger.debug(
                    f"Synced final batch of {len(documents)} movies; total synced: {total_synced}"
                )
    ...
    def search_movies(
        self, query: str, per_page: int = 10, page: int = 1, genre_filter: str = None
    ) -> List[Dict[str, Any]]:
        search_params = {
            "q": query,
    ...
    def batch_index_movies(
        self, movies: List[Dict[str, Any]], batch_size: int = 1000
    ) -> None:
        if not movies:
